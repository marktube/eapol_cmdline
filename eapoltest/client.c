...
#pragma mark Functions declaration of private framework

int EAPOLControlCopyStateAndStatus(const char*, EAPOLControlState*, CFDictionaryRef*);
int EAPOLControlStart(const char * interface_name, CFDictionaryRef config_dict);
int EAPOLControlStop(const char * interface_name);
int EAPOLControlRetry(const char * interface_name);
int EAPOLControlUpdate(const char * interface_name, CFDictionaryRef config_dict);
int EAPOLControlProvideUserInput(const char * interface_name, CFDictionaryRef user_input);
CFStringRef EAPOLControlAnyInterfaceKeyCreate(void);
CFStringRef EAPOLControlKeyCreate(const char * interface_name);
CFStringRef EAPOLControlKeyCopyInterface(CFStringRef key);
Boolean EAPOLControlPrefsSetLogFlags(uint32_t flags);
OSStatus EAPSecIdentityListCreate(CFArrayRef * ret_array);
CFStringRef EAPSecCertificateCopyUserNameString(SecCertificateRef cert);
EAPSecIdentityHandleRef EAPSecIdentityHandleCreate(SecIdentityRef identity);
int EAPOLControlStartSystem(const char * interface_name, CFDictionaryRef options);
int EAPOLControlCopyLoginWindowConfiguration(const char * interface_name, CFDictionaryRef * ret_config_p);
int EAPOLControlCopyAutoDetectInformation(CFDictionaryRef * info_p);
void EAPOLControlSetUserAutoConnectEnabled(Boolean enable);
Boolean EAPOLControlIsUserAutoConnectEnabled(void);
boolean_t EAPOLControlDidUserCancel(const char * interface_name);

Boolean SCDynamicStoreNotifyWait(SCDynamicStoreRef store);
void SCPrint(Boolean, FILE*, CFStringRef, int, CFStringRef);

#pragma mark Functions declaration for local use

static void command_usage();
static void usage();
static int S_state(int argc, char* argv[]);
static int S_start(int argc, char* argv[]);
static int S_stop(int argc, char* argv[]);
static int S_retry(int argc, char* argv[]);
static int S_update(int argc, char* argv[]);
static int S_input(int argc, char* argv[]);
static int S_monitor(int argc, char* argv[]);
static int S_set_verbose(int argc, char* argv[]);
static int S_stress_start(int argc, char* argv[]);
static int S_show_identities(int argc, char* argv[]);
static int S_start_system(int argc, char* argv[]);
static int S_loginwindow_config(int argc, char* argv[]);
static int S_show_autodetect_info(int argc, char* argv[]);
static int S_set_user_autoconnect(int argc, char* argv[]);
static int S_get_user_autoconnect(int argc, char* argv[]);
static int S_did_user_cancel(int argc, char* argv[]);
